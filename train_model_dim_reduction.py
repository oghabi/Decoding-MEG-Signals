...
#2000 is not enough for it to learn. PCA with 7000 could learn
...
# x_test = pca.transform(x_test)   #only use transform on test data

input_img = Input(shape=(dim*win_len,))

# encoded = Dense(6000, activation='relu')(input_img)
...
# decoded = Dense(dim*win_len, activation='sigmoid')(decoded)

#One layer Auto-Encoder
encoded = Dense(encoding_dim, activation='relu')(input_img)
decoded = Dense(dim*win_len, activation='sigmoid')(encoded)
...
#Using AE trained with training data
x_train = encoder.predict(x_train)
x_test = encoder.predict(x_test)



print("X_train and X_test shape after Dimensionality reduction: ", x_train.shape, x_test.shape)
...
def model_MLP(model, num_hidden_units):
	model.add(Dense(num_hidden_units, activation='relu', input_shape=(x_train.shape[1],), kernel_initializer='he_uniform'))
...
network_type = 'MLP';  model_patience = 7;
